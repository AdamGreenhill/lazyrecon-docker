...
RUN apt update && \
        ...
        wget https://raw.githubusercontent.com/AdamGreenhill/theGreatRecon/main/theGreatSetup-docker.sh && \
        chmod +x theGreatSetup-docker.sh && \
        bash theGreatSetup-docker.sh && \
        mkdir -p /opt/lazyrecon/ && \
        cd /opt/lazyrecon/ && \
        wget https://raw.githubusercontent.com/AdamGreenhill/lazyrecon/master/lazyrecon.sh && \
